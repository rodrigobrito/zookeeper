namespace Org.Apache.Zookeeper.Data
{
    ...
    public class StatPersistedV1 : IRecord, IComparable
    {
        ...
        {
            StatPersistedV1 dv = (StatPersistedV1) obj;
            if (dv == null)
            {
                throw new InvalidOperationException("Comparing different types of records.");
            }
            int num = 0;
            num = (this.Czxid == dv.Czxid) ? 0 : ((this.Czxid < dv.Czxid) ? -1 : 1);
            if (num == 0)
            {
                num = (this.Mzxid == dv.Mzxid) ? 0 : ((this.Mzxid < dv.Mzxid) ? -1 : 1);
                if (num != 0)
                {
                    return num;
                }
                num = (this.Ctime == dv.Ctime) ? 0 : ((this.Ctime < dv.Ctime) ? -1 : 1);
                if (num != 0)
                {
                    return num;
                }
                num = (this.Mtime == dv.Mtime) ? 0 : ((this.Mtime < dv.Mtime) ? -1 : 1);
                if (num != 0)
                {
                    return num;
                }
                num = (this.Version == dv.Version) ? 0 : ((this.Version < dv.Version) ? -1 : 1);
                if (num != 0)
                {
                    return num;
                }
                num = (this.Cversion == dv.Cversion) ? 0 : ((this.Cversion < dv.Cversion) ? -1 : 1);
                if (num != 0)
                {
                    return num;
                }
                num = (this.Aversion == dv.Aversion) ? 0 : ((this.Aversion < dv.Aversion) ? -1 : 1);
                if (num != 0)
                {
                    return num;
                }
                num = (this.EphemeralOwner == dv.EphemeralOwner) ? 0 : ((this.EphemeralOwner < dv.EphemeralOwner) ? -1 : 1);
                if (num != 0)
                {
                    return num;
                }
        ...
        {
            StatPersistedV1 objA = (StatPersistedV1) obj;
            if (objA == null)
            {
                return false;
            }
